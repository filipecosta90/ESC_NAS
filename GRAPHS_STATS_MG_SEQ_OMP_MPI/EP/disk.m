...
disk_read_seq = disk_read_seq ./ 10^6;
disk_write_seq = disk_write_seq ./ 10^6;
...
disk_read_omp = disk_read_omp ./ 10^6;
disk_write_omp = disk_write_omp ./ 10^6;
...
disk_read_mpi = disk_read_mpi ./ 10^6;
disk_write_mpi = disk_write_mpi ./ 10^6;
...
ax1 = subplot(3,1,[1 2]);
plot(disk_read_seq,'Color', cores(1,:),'LineWidth',3);
hold on;

plot(disk_read_omp,'Color', cores(2,:),'LineWidth',3);
hold on;

plot(disk_read_mpi,'Color', cores(3,:),'LineWidth',3);
hold on;

plot(disk_write_seq,'--','Color', cores(6,:),'LineWidth',3);
hold on;

plot(disk_write_omp,'--','Color', cores(4,:),'LineWidth',3);
hold on;

plot(disk_write_mpi,'--','Color', cores(5,:),'LineWidth',3);
hold on;
set(gca,'YTickLabel',num2str(get(gca,'YTick').'));

xlim(ax1,[0 18]);

ylabel('Disk Read/Write (MB)');
xlabel('Tempo (s)');
l = legend('Disk READ - EP - SEQ','Disk READ - EP - OMP','Disk READ - EP - MPI','Disk WRITE - EP - SEQ','Disk WRITE - EP - OMP','Disk WRITE - EP - MPI');

title({'Disk Stats, kernel EP, Tempo entre [0,18]'},'interpreter','latex')
...
title({'Disk Stats, kernel EP'},'interpreter','latex')
