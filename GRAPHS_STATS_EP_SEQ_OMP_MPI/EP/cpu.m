...
%%%%%% CPU STATS %%%%%
cpu_usr_seq = table2array( cpu_csv_seq ( :, [3])); 
...
%%%%%% CPU STATS %%%%%
cpu_usr_omp = table2array( cpu_csv_omp ( :, [3])); 
...
%%%%%% CPU STATS %%%%%
cpu_usr_mpi = table2array( cpu_csv_mpi ( :, [3])); 
...
ax1 = subplot(3,1,[1 2]);
plot(cpu_usr_seq,'Color', cores(1,:),'LineWidth',3);
hold on;

plot(cpu_usr_omp,'Color', cores(2,:),'LineWidth',3);
hold on;

plot(cpu_usr_mpi,'Color', cores(3,:),'LineWidth',3);
hold on;
xlim(ax1,[0 18]);

ylabel('Percentagem');
xlabel('Tempo (s)');
l = legend('% CPU USR - EP - SEQ','% CPU USR - EP - OMP','% CPU USR - EP - MPI' );

title({'CPU Stats, kernel EP, Tempo entre [0,18]'},'interpreter','latex')
...
title({'CPU Stats, kernel EP'},'interpreter','latex')
