...
ax1 = subplot(3,1,[1 2]);
plot(net_recv_seq,'Color', cores(1,:),'LineWidth',3);
hold on;

plot(net_recv_omp,'Color', cores(2,:),'LineWidth',3);
hold on;

plot(net_recv_mpi,'Color', cores(3,:),'LineWidth',3);
hold on;

plot(net_send_seq,'Color', cores(6,:),'LineWidth',3);
hold on;

plot(net_send_omp,'Color', cores(4,:),'LineWidth',3);
hold on;

plot(net_send_mpi,'Color', cores(5,:),'LineWidth',3);
hold on;
set(gca,'YTickLabel',num2str(get(gca,'YTick').'));

xlim(ax1,[0 40]);

ylabel('Receive/Send (MB)');
xlabel('Tempo (s)');
l = legend('NET Receive - CG - SEQ','NET Receive - CG - OMP','NET Receive - CG - MPI','NET Send - CG - SEQ','NET Send - CG - OMP','NET Send - CG - MPI');

title({'Net Stats, kernel CG, Tempo entre [0,40]'},'interpreter','latex')
...
title({'Net Stats, kernel CG'},'interpreter','latex')
